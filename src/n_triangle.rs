...
///
/// 高さが 4の時、高さが2の逆三角形が 1
/// 高さが 5の時、高さが2の逆三角形が 3
/// 高さが 6の時、高さが2の逆三角形が 6, 高さが3の逆三角形が 1
/// 高さが 7の時、高さが2の逆三角形が10, 高さが3の逆三角形が 3
/// 高さが 8の時、高さが2の逆三角形が15, 高さが3の逆三角形が 6, 高さが4の逆三角形が1
/// 高さが 9の時、高さが2の逆三角形が21, 高さが3の逆三角形が10, 高さが4の逆三角形が3
/// 高さが10の時、高さが2の逆三角形が28, 高さが3の逆三角形が15, 高さが4の逆三角形が6, 高さが5の逆三角形が1
///
/// 高さが10の時、
///   高さ10の時に現れる高さ5の逆三角形の合計:  1
///   高さ 8の時に現れる高さ4の逆三角形の合計:  6
///   高さ 6の時に現れる高さ3の逆三角形の合計: 15
///   高さ 4の時に現れる高さ2の逆三角形の合計: 28
///
/// このことから、{an} は[1, 6, 15, 28, 45] である。
///   {an} の項数は、((高さ - 4(逆三角形が現れる数) + 1) + (高さ % 2)) / 2 を n とする
///     高さから逆三角形が現れる数の4を引き、その中から奇数の数を求める
///
///  {an} の階差数列 {bn} は、[5, 9, 13, 17]。
///
/// 等差数列の一般項の公式を用いて {bn} を解く
///   初項 5, 公差 4 であるから、
///     bn = a + (n−1) * d
...
///     an = a1 + ∑(k=1, n-1, bk)
///        = 1 + ∑(k=1, n-1, 4k + 1)
///        = 1 + ∑(k=1, n-1, 4k) + ∑(k=1, n-1, 1)
///        = 1 + 4∑(k=1, n-1, k) + ∑(k=1, n-1, 1)
///        = 1 + 4 * ((n-1)n) / 2 + n - 1
///        = 1 + 2n(n-1) + n - 1
///        = 2n(n-1) + n
///        = 2n^2 - 2n + n
...
///        = 2*(n(n+1)(2n+1))/6 - (3n(n+1))/2*3
///        = (2n(n+1)(2n+1) - 3n(n+1)) / 6
///        = (2n(n+1)(2n+1) - 3n(n+1)) / 6
///        = ((2n^2+2n)(2n+1) - (3n^2 + 3n)) / 6
///        = (4n^3 + 6n^2 + 2n - 3n^2 - 3n) / 6
///        = (4n^3 + 3n^2 - n) / 6
///
/// 以上の計算結果から、 (4n^3 + 3n^2 - n) / 6 で高さが2以上の逆三角形が求められる。
fn inverted_triangles_with_height_greater_than_or_equal_to_2(n: u64) -> u64 {
    (4 * n.pow(3) + 3 * n.pow(2) - n) / 6
}
